...
export default function MEReviewPanel({
  ...
  function startPolling() {
    ...
    setTimeout(poll, delay);
    // Return a cleanup fn — we keep a ref to cancelled so concurrent calls
      ...
      <div className="px-4 py-4">
        ...
        {status === "done" && review && (
          <div className="prose prose-sm max-w-none prose-headings:font-semibold"
            style={{ "--tw-prose-headings": "var(--amber-heading)", "--tw-prose-bold": "var(--amber-heading)" } as React.CSSProperties}
          >
            {activeTab === "technical" ? (
              ...
              <div>
                ...
                <Markdown>{simplified ?? "(No simplified explanation available.)"}</Markdown>
              </div>
            )}
          </div>
        )}
